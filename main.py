...
def remove_code_blocks(text):
    ...
    # Удаляем полноценные Markdown-кодовые блоки
    text = re.sub(
        r"```(?:[a-zA-Z0-9_+#.-]+)?"
        r"\s*\n?.*?```",
        "",
    ...
    # Если модель случайно написала очевидный
    # маркер языка в отдельной строке — убираем его.
    text = re.sub(
        r"(?mi)^\s*(python|javascript|typescript|java|"
        r"javascript|html|css|sql|bash|json|php|c\+\+|c#)\s*$",
        "",
        text
    )
...
def ask_ai_with_history(
    ...
    if user_id not in user_histories:
        ...
        if mode == "neuroham":
            ...
            sys_prompt = (
                ...
                "КРИТИЧЕСКОЕ ПРАВИЛО: "
                "если пользователь НЕ просит написать, "
                "исправить, объяснить или показать "
                "программный код, НЕ ПИШИ КОД "
                "И НЕ ВЫВОДИ КОДОВЫЕ БЛОКИ. "

                "Не добавляй ```python, ``` или "
                "другие кодовые блоки по собственной "
                "инициативе. "

                "Если пользователь действительно "
                "просит программный код, тогда код "
                "можно показать в отдельном кодовом блоке."
            )
    ...
    if mode == "neuroham":
        ...
        if is_coding_request:
            ...
            messages_to_send[-1]["content"] = (
                ...
                "Без мата. "
                "Объяснение без Markdown. "
                "Если нужен код — код помещай "
                "в отдельные кодовые блоки.]\n\n"
                +
                messages_to_send[-1]["content"]
            )
            ...
            messages_to_send[-1]["content"] = (
                "[ОБЯЗАТЕЛЬНО: это НЕ запрос на код. "
                "Ответь только обычным текстом. "
                "НЕ используй ``` и НЕ пиши "
                "никакой программный код. "
                "Стиль — саркастичный и ворчливый Нейрохам. "
    ...
    if success:
        ...
        # ====================================================
        # ЗАЩИТА НЕЙРОХАМА ОТ СЛУЧАЙНОГО КОДА
        # ====================================================

        if mode == "neuroham" and not is_coding_request:

            answer = remove_code_blocks(
                answer
            )
...
def is_kira_question(text):
    ...
    normalized = normalize_kira_text(
        text
    )
    ...
    # ========================================================
    # ЯВНЫЙ КОНТЕКСТ ДРУГОЙ КИРЫ
    #
    # Если пользователь уточнил персонажа,
    # пасхалка НЕ должна срабатывать.
    # ========================================================
    ...
    if any(
        ...
        for phrase in other_kira_context
    ):

        return False

    # ========================================================
    # ПРЯМЫЕ ВОПРОСЫ ДЛЯ ПАСХАЛКИ
    # ========================================================
    ...
    exact_patterns = [
        ...
        r"^расскажи про киру$",
        r"^расскажи кто такая кира$",

        r"^можешь рассказать про киру$",
        r"^можешь рассказать кто такая кира$"
    ]
    ...
    # ========================================================
    # ВАРИАНТЫ С ВОПРОСИТЕЛЬНЫМИ СЛОВАМИ
    # ========================================================

    if re.fullmatch(
        r"(а\s+)?кто\s+такая\s+кира",
        normalized
    ):
...
def handle_text(message):
    ...
    if is_kira_question(text):
        ...
        try:
            ...
            # ВАЖНО:
            # Пасхалка по-прежнему вызывает отдельную
            # AI-функцию generate_kira_text().
