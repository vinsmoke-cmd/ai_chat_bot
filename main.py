import io, os, re, sys, glob, json, math, time, asyncio, logging, requests, urllib.parse
from PIL import Image
import docx, pptx, openpyxl, fitz
...
# --- ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ И ПОГОДА ---
def get_weather_data(location_name: str) -> str:
    try:
        ...
        # Поиск по названию
        res = requests.get(geo_url, params={'name': location_name, 'count': 1, 'language': 'ru', 'format': 'json'}, headers=headers, timeout=5).json()
        if not res.get('results'):
            res = requests.get(geo_url, params={'name': location_name, 'count': 1, 'format': 'json'}, headers=headers, timeout=5).json()
            
        if not res.get('results'):
            return f"Не удалось найти город '{location_name}'. Проверьте написание."
            
        city = res['results'][0]
        lat, lon, name = city['latitude'], city['longitude'], city.get('name', location_name)
        ...
        if ext == '.pdf':
            ...
            for sheet in wb.worksheets:
                for row in sheet.iter_rows(values_only=True):
                    ...
                    if row_str: text.append(row_str)
            return "\n".join(text)
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "Или просто отправьте мне файл, текст или голосовое сообщение!"
    )
...
async def handle_gen_doc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not prompt: return await update.message.reply_text("Укажите тему документа.")
    msg = await update.message.reply_text("Генерирую документ...")
    model = genai.GenerativeModel('gemini-2.0-flash')
    res = model.generate_content(f"Напиши подробный документ на тему: {prompt}")
    buf = create_docx(res.text)
...
async def handle_gen_pptx(update: Update, context: ContextTypes.DEFAULT_TYPE):
    prompt = " ".join(context.args)
    if not prompt: return await update.message.reply_text("Укажите тему презентации.")
    msg = await update.message.reply_text("Генерирую презентацию...")
    model = genai.GenerativeModel('gemini-2.0-flash')
    res = model.generate_content(f"Создай структуру слайдов PPTX на тему: {prompt}. Разделяй слайды символом '---'. Первый заголовок слайда, дальше пункты.")
    buf = create_pptx(res.text)
...
async def handle_gen_excel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    prompt = " ".join(context.args)
    if not prompt: return await update.message.reply_text("Укажите описание таблицы.")
    msg = await update.message.reply_text("Генерирую таблицу...")
    model = genai.GenerativeModel('gemini-2.0-flash')
    res = model.generate_content(f"Сформируй данные для таблицы Excel на тему: {prompt}. Разделяй колонки символом '|', строки переносом.")
    buf = create_xlsx(res.text)
...
async def handle_gen_image(update: Update, context: ContextTypes.DEFAULT_TYPE):
    prompt = " ".join(context.args)
    if not prompt: return await update.message.reply_text("Укажите промпт для генерации картинки.")
...
async def process_natural_language_request(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    ...
    # Погода через естественный язык
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text: await process_natural_language_request(update, context, text)
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not text: return await update.message.reply_text("Не удалось прочитать файл.")
    
    prompt = update.message.caption or "Сделай краткую выжимку из этого файла."
    model = genai.GenerativeModel('gemini-2.0-flash')
    res = model.generate_content(f"Содержимое файла {doc.file_name}:\n{text[:10000]}\n\nЗадание: {prompt}")
