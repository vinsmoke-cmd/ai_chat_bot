    ...
    for url in urls:
        try:
            ...
            with urllib.request.urlopen(
                req,
                timeout=10
            ) as resp, open(font_path, 'wb') as f:
                f.write(resp.read())
            ...
            print(
                f"⚠️ Ошибка загрузки шрифта {url}: {e}"
            )
...
def create_generated_file(request, user_id):
    ...
    if (
        ...
        and (
            ...
            or "таблица умножения" in request_lower
        )
    ):
    ...
    if extension == "xlsx":
        ...
        if font_file:
            try:
                ...
                print(
                    f"⚠️ Ошибка регистрации шрифта: {e}"
                )
...
def send_code_block(
    ...
    try:
        ...
        print(
            f"⚠️ Ошибка отправки кодового блока: {e}"
        )
        ...
        try:
            ...
            print(
                f"❌ Ошибка fallback кодового блока: {e2}"
            )
...
def edit_or_send_long(
    ...
    try:
        ...
        print(
            f"⚠️ Не удалось удалить временное сообщение: {e}"
        )
...
def ask_ai_with_history(
    ...
    return (
        ...
        if mode == "neuroham"
        else "Не удалось получить ответ ни от одной ИИ-модели. "
...
def generate_kira_text():
    ...
    return (
        "Кира — словно редкая виниловая пластинка "
        "с любимой музыкой. Она приносит с собой "
        "особый ритм и уют. ✨❤️"
    )
...
def enhance_image_prompt(user_prompt):
    try:
        sys_prompt = (
            ...
            "Expand the user's request into a highly detailed, "
            "vivid, beautiful image description in English. "
            "Add specifics about lighting, textures, composition, "
            "camera, atmosphere and visual style. "
            "Do not change the main idea of the user. "
            "Return ONLY the enhanced English prompt without "
        ...
        print(
            f"⚠️ Ошибка улучшения промпта: {e}"
        )
...
def generate_image_dynamic(prompt):
    if not POLLINATIONS_API_KEY:
        print(
            "❌ POLLINATIONS_API_KEY не установлен"
        )
...
def stats_cmd(message):
    msg = (
        ...
        f"{len(stats['users'])}\n"
        f"Сгенерировано фото: "
        f"{stats['images_generated']}\n"
        f"Создано файлов: "
        f"{stats['files_generated']}\n"
        f"Распознано голосовых: "
        f"{stats['voice_messages']}"
    )
...
def execute_file_creation(
    ...
    try:
        path, filename, extension = create_generated_file(
            request_text,
            message.chat.id
        )
...
def fact_cmd(message):
    topic = clean_command_args(
        message.text,
        "fact"
    )

    prompt = (
        f"Расскажи интересный факт на тему: {topic}. "
        f"Будь краток."
        if topic
        else "Расскажи случайный интересный факт."
    )
...
def execute_search(
    ...
    prompt = (
        ...
        f"'{query}':\n\n"
        f"{raw_data}\n\n"
        f"Сделай краткую выжимку. Без Markdown."
    )
...
@bot.message_handler(
    commands=["gemini", "code", "sum", "tr", "fix"]
)
    ...
    if image_bytes:
        ...
        markup.add(
            types.InlineKeyboardButton(
                "🔄 Перегенерировать",
                callback_data=f"reimage:{image_id}"
            )
        )
...
@bot.callback_query_handler(
    func=lambda call: call.data.startswith("reimage:")
)
def callback_reimage(call):
    image_id = call.data.split(
        "reimage:",
        1
    )[1]

    prompt = image_prompts.get(
        image_id
    )
    ...
    if not prompt:
        bot.answer_callback_query(
            call.id,
            "❌ Данные изображения уже недоступны.",
            show_alert=True
        )
...
def tts_cmd(message):
    text_to_speak = clean_command_args(
        message.text,
        "tts"
    )
    ...
    if (
        ...
        try:
            ...
            if (
                ...
                raise ValueError(
                    "Не удалось получить текст через Jina AI."
                )

            reply = ask_ai_with_history(
                message.chat.id,
                "Сделай краткую выжимку этого текста. "
...
def handle_doc(message):
    ...
    try:
        ...
        if not text.strip():
            raise ValueError(
                "Документ пуст или текст не распознан."
            )

        reply = ask_ai_with_history(
            message.chat.id,
            "Сделай краткую и понятную выжимку "
            "из этого документа. Не используй Markdown:\n\n"
            + text[:6000]
        )
